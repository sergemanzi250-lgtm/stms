...
async function createSchoolTimeSlots(schoolId, schoolName = 'Unknown School') {
  console.log(`Creating time slots for school: ${schoolName} (${schoolId})`)

  // First, delete existing time slots for this school
  ...
  for (const day of daysOfWeek) {
    ...
    timeSlots.push({
      schoolId,
      day,
      period: -1, // Use negative for breaks
    ...
    timeSlots.push({
      schoolId,
      day,
      period: -2, // Use negative for breaks
      name: 'Morning Break',
    ...
    timeSlots.push({
      schoolId,
      day,
      period: -3, // Use negative for breaks
      name: 'Lunch Break',
    ...
    timeSlots.push({
      schoolId,
      day,
      period: -4, // Use negative for breaks
  ...
  try {
    ...
    console.log(`✅ Created ${createdSlots.count} time slots for school ${schoolName}`)
...
// Get school ID from command line argument or use default
const schoolId = process.argv[2] || 'cmjl490ak00013n3xx9exbihy' // GS REMERA A TSS
const schoolName = process.argv[3] || 'GS REMERA A TSS'

async function main() {
  console.log('Setting up time slots for school with missing time slots...')
  const result = await createSchoolTimeSlots(schoolId, schoolName)
  console.log('Setup result:', result)
  await prisma.$disconnect()
  
  if (result.success) {
    console.log('\n🎉 Time slots setup completed successfully!')
    console.log('The school can now generate timetables.')
  } else {
    console.log('\n❌ Time slots setup failed!')
  }
}
