...
export default function ClassTimetables() {
    ...
    // Filter timetables by class and search term
    ...
    const classTimetables = Array.from(new Set(filteredTimetables.map(t => t.classId))).map(classId => {
        const classTimetable = filteredTimetables.filter(t => t.classId === classId)
        return {
            id: classId,
    ...
    const getTimetableGrid = (classEntries: TimetableEntry[]) => {
    ...
    const getCellContent = (entry: TimetableEntry | null) => {
        ...
        // For class view: show subject/module + teacher
        return { subjectName, teacherName, isSubject, category }
    }
    ...
    const getSubjectColor = (isSubject: boolean, category: string, subjectName?: string) => {
        if (isSubject) {
            ...
            return subjectColors[(subjectName || '').toUpperCase() as keyof typeof subjectColors] || 'bg-gray-100 text-gray-800 border-gray-200'
        } else {
            ...
            return moduleColors[category.toUpperCase() as keyof typeof moduleColors] || moduleColors.default
        }
    }
        ...
        <div className="min-h-screen bg-gray-50">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                        ...
                        <div className="text-sm text-gray-600">
                            {classTimetables.length} classes found
                        </div>
                    </div>
            ...
            <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
                <div className="px-4 py-6 sm:px-0">
                    {viewMode === 'list' ? (
                        // List View - Show all classes with their timetables
                        <div className="bg-white shadow rounded-lg overflow-hidden">
                            <div className="px-6 py-4 border-b border-gray-200">
                                <h2 className="text-xl font-bold text-gray-900">All Class Timetables</h2>
                                <p className="text-sm text-gray-600">
                                    {classTimetables.length} classes with generated timetables
                                </p>
                            </div>
                            <div className="p-6">
                                {classTimetables.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {classTimetables.map((classTimetable) => (
                                            <div
                                                key={classTimetable.id}
                                                className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow"
                                            >
                                                <div className="flex items-start justify-between mb-4">
                                                    <div>
                                                        <h3 className="text-lg font-semibold text-gray-900">{classTimetable.name}</h3>
                                                        {classTimetable.level && (
                                                            <p className="text-sm text-gray-600">Level: {classTimetable.level}</p>
                                                        )}
                                                        <p className="text-sm text-gray-500 mt-1">
                                                            {classTimetable.entries.length} periods scheduled
                                                        </p>
                                                    </div>
                                                ...
                                                <div className="space-y-2 mb-4">
                                                    <p className="text-xs text-gray-500">
                                                        Generated: {new Date(classTimetable.createdAt).toLocaleDateString()}
                                                    </p>
                                                </div>

                                                <div className="flex space-x-2">
                                                    <SinglePDFExportButton
                                                        entries={classTimetable.entries.map(t => ({
                                                            id: t.id,
                                                            day: t.timeSlot.day,
                                                        ...
                                                        title={`${session.user.schoolName} - Class Timetable - ${classTimetable.name}`}
                                                        schoolName={session.user.schoolName || undefined}
                                                        onExportStart={() => console.log('PDF export started for class:', classTimetable.name)}
                                                        onExportComplete={() => console.log('PDF export completed for class:', classTimetable.name)}
                                                        onExportError={(error) => console.error('PDF export failed for class:', classTimetable.name, error)}
                                                    />
                                                </div>
                                            </div>
                                        ))}
                        ...
                        // Regular/Compact View - Show individual class timetables
                        <div className="space-y-8">
                            {classTimetables.length === 0 ? (
                                ...
                                classTimetables.map((classTimetable) => {
                                    const timetableGrid = getTimetableGrid(classTimetable.entries)

                                    return (
                                        <div key={classTimetable.id} className="bg-white shadow rounded-lg overflow-hidden print:shadow-none print:border print:border-gray-300">
                                            <div className="px-6 py-4 border-b border-gray-200 print:px-4 print:py-2">
                                                <div className="flex justify-between items-center">
                                                    <div>
                                                        <h2 className="text-2xl font-bold text-gray-900 print:text-xl">{classTimetable.name}</h2>
                                                        {classTimetable.level && (
                                                            <p className="text-sm text-gray-600 print:text-xs">Level: {classTimetable.level}</p>
                                                        )}
                                                        <p className="text-sm text-gray-500 print:text-xs">
                                                            {classTimetable.entries.length} periods scheduled • Generated: {new Date(classTimetable.createdAt).toLocaleDateString()}
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center space-x-2 no-print">
                                                            ...
                                                            <CSVGridExportButton
                                                                ...
                                                                title={`${session.user.schoolName} - Class Timetable - ${classTimetable.name}`}
                                                                onExportStart={() => console.log('CSV export started for class:', classTimetable.name)}
                                                                onExportComplete={() => console.log('CSV export completed for class:', classTimetable.name)}
                                                                onExportError={(error) => console.error('CSV export failed for class:', classTimetable.name, error)}
                                                            />
                                                        </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="p-6 print:p-4">
                                                {viewMode === 'compact' ? (
                                                    <CompactA4Timetable
                                                        ...
                                                        title={`${classTimetable.name} Timetable`}
